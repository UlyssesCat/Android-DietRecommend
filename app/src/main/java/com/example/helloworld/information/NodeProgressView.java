...
public class NodeProgressView extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mNodeProgressColor = mTypedArray.getColor(R.styleable.nodeProgress_node_progresscolor, Color.BLUE);

        mTextColor = mTypedArray.getColor(R.styleable.nodeProgress_node_tip, Color.BLACK);
        mTextProgressColor = mTypedArray.getColor(R.styleable.nodeProgress_node_progress_tip, Color.BLACK);

        mLineColor = mTypedArray.getColor(R.styleable.nodeProgress_node_bar, Color.GRAY);
        mLineProgressColor = mTypedArray.getColor(R.styleable.nodeProgress_node_progress_bar, Color.BLUE);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < mNumber; i++) {
            ...
            if (i == mNumber - 1) {
                ...
                node.setPoint(paint);
                switch (i){
                    case 0 : node.setDes("低热量");
                    case 1 : node.setDes("中热量");
                    case 2 : node.setDes("高热量");
                }
                nodes.add(node);
                continue;
            }
